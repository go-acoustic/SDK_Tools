...
window.TLT = (function () {
              ...
              core.utils.clog("Aborting initialization because the killswitch is active.");
              return;
              }
              ...
              if (killswitchURL) {
              core.utils.clog("Checking killswitch: " + killswitchURL);
              ajaxService.sendRequest({
                                      ...
                                      core.utils.clog("Shutting down because killswitch engaged.");
              ...
              ajaxService.sendRequest({
                                      ...
                                      //do nothing
                                      if (result.success) {
                                      core.utils.clog("Endpoint check passed: ", result);
                                      }
                                      },
                                      ...
                                      core.utils.clog("Endpoint check failed: ", result);
                                      core.setAutoFlush(false);
                                      core.destroy();
                                      }
                                      });
...
/*jshint devel:true */
               ...
               utils.clog("UIC self-terminated due to inactivity(1).");
               return;
               }
               ...
               utils.clog("Cannot access xdomain frame window.");
               return;
               }
               requestData = {
               request: {
               ...
               utils.clog("Cannot access sendMessage API on xdomain frame window.");
               return;
               }
               }
               } else {
               ...
               utils.clog("UIC self-terminated due to inactivity(2).");
               core.destroy();
               return;
               }
               }
               ...
               utils.clog("Attempt to initialize service which has been already initialized(queueService)");
               }
               },
               ...
               if (!queueManager.exists(queueId)) {
               utils.clog("Queue: " + queueId + " does not exist!");
               return;
               }
...
TLT.addService("browser", function (core) {
               ...
               utils.clog("Attempt to initialize service which has been already initialized(browserService.jQuery)");
               }
               },
...
TLT.addService("ajax", function (core) {
               ...
               utils.clog("Attempt to initialize service which has been already initialized(ajaxService.jQuery)");
               }
               },
...
TLT.addService("domCapture", function (core) {
               ...
               if (!forceFullDOM) {
               utils.clog("Switching to full DOM capture as total mutations (" + mutationCount +
                          ") matched or exceeded configured threshold (" + mutationThreshold + ")");
               forceFullDOM = true;
               }
               return;
               }
...
/*global TLT:true */
               ...
               TLT.addModule("overstat", function (context) {
                             ...
                             utils.clog("Overstat - posted hover event");
                             utils.clog(uiEvent);
